...
abstract interface class CrudRepository<T, Q> {
  ...
  /// Отмена незавершённой выборки: её результат уже не нужен.
  ///
  /// Пользователь набирает «принтер» по букве — уходит семь запросов,
  /// и ответ на «пр» может прийти после ответа на «принтер». Тип токена
  /// отмены сюда не выносится намеренно: договор не должен знать,
  /// какой библиотекой выполняется запрос. Локальная реализация ничего
  /// не делает — там отменять нечего.
